...
/*
TF1 JetTriggerEfficiencies::findfunction(const int & sigma, const float & eta)
{      
      double etalimits[4] = {0.0, 1.0, 1.4, 2.2} ; 
      std::string etabins[3] = {"0p0_to_1p0","1p0_to_1p4","1p4_to_2p2"};
      std::string etabin = "";
      ...
      {
         var = "nominal";
         function_name = Form("jetOnlineTriggerScaleFactor_etabin_%s_%s",etabin.c_str(), var.c_str());
      }
      ...
      else
      {
         ...
         if (sigma < 0)
         var = "down";
         function_name = Form("jetOnlineTriggerScaleFactor_etabin_%s_%ds_%s",etabin.c_str(),int(fabs(sigma)), var.c_str());
      }

      TF1 funct_sf = *((TF1*)f -> Get(function_name.c_str()));
